document.addEventListener('DOMContentLoaded', function () {
    // Simulated food data (replace this with your actual data)
    const foodData = [
        {
            id: 1,
            name: 'Fushka',
    ...
    const foodPaginationContainer = document.getElementById('food-pagination');
    const foodCount = document.getElementById('food-count');

    foodCount.textContent = foodData.length;

    const displayFoods = (page) => {
        foodList.innerHTML = '';
        const startIndex = (page - 1) * foodsPerPage;
        const endIndex = Math.min(startIndex + foodsPerPage, foodData.length);
    ...
    const onPageChangeFood = (page) => {
        currentFoodPage = page;
        displayFoods(currentFoodPage);
        updatePagination(foodPaginationContainer, Math.ceil(foodData.length / foodsPerPage), currentFoodPage, onPageChangeFood);
    };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const reviewData = [
      {
        ...
        reply: 'Thank you for your feedback. We look forward to serving you again!'
      },
      {
        customerId: 2,
        ...
        reply: 'We are glad you enjoyed our offerings. Looking forward to your next visit!'
      },
      ...
      },
      {
        ...
        reply: 'Thank you for your feedback, Bob. We will work on improving our food.'
      },
      {
        customerId: 5,
        customerName: 'Arif ',
        ...
        reply: 'Thank you for your feedback, Charlie. We will strive to do better.'
      }
      // More reviews...
    ];
    ...
    const updatePagination = () => {
      ...
      paginationContainer.appendChild(createPageItem(currentPage - 1, false, currentPage === 1, 'Previous'));
      for (let i = 1; i <= totalPages; i++) {
        paginationContainer.appendChild(createPageItem(i, currentPage === i, false));
      }
      paginationContainer.appendChild(createPageItem(currentPage + 1, false, currentPage === totalPages, 'Next'));
    };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const bloggerPaginationContainer = document.getElementById('blogger-pagination');
    ...
    const displayBloggers = (page) => {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        row.innerHTML = `
          <td>${i + 1}</td>
          <td><a href="${blogger.profileUrl}" target="_blank">${blogger.customerId}</a></td>
          <td>${blogger.bloggerName}</td>
          <td>${blogger.channelName || 'N/A'}</td>
          <td><a href="${blogger.videoUrl}" target="_blank">${blogger.videoTitle}</a></td>
          <td>${blogger.videoDescription}</td>
          <td><textarea data-id="${blogger.customerId}">${blogger.reply}</textarea></td>
          <td><button class="btn-reply" data-id="${blogger.customerId}">Reply</button></td>
        `;
      ...
      replyButtons.forEach(button => {
        button.addEventListener('click', (event) => {
          ...
          const blogger = bloggerData.find(b => b.customerId == customerId);
          if (blogger) {
            alert(`Reply sent for Customer ID: ${customerId}\nReply: ${blogger.reply}`);
          }
        });
      });
    };
    ...
    const onPageChangeBlogger = (page) => {
      ...
      updatePagination(bloggerPaginationContainer, Math.ceil(bloggerData.length / bloggersPerPage), currentBloggerPage, onPageChangeBlogger);
    };
